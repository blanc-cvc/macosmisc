...
if [[ "$(id -u)" == "0" ]]; then
    ...
    echo "##"
    /bin/bash "$SCRIPT_DIR/etc/macosmisc/tools/makemobileconfig.restrictions.sh"
    /bin/bash "$SCRIPT_DIR/etc/macosmisc/tools/makemobileconfig.apn.orange.sh"
    /bin/bash "$SCRIPT_DIR/etc/macosmisc/tools/makemobileconfig.ios.builtinfilter.sh"
    /bin/bash "$SCRIPT_DIR/etc/macosmisc/tools/makemobileconfig.dns.sh"
    echo "                  double click on it from Finder macos:(dns and restrictions only)"
    echo "#"
