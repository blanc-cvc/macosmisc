...
_dhcpcomment() {
    /etc/.custom/pfmanager.sh --tag @DHCP_IN --action COMMENT
    /etc/.custom/pfmanager.sh --tag @DHCP_OUT --action COMMENT
}
_dhcpuncomment() {
    /etc/.custom/pfmanager.sh --tag @DHCP_IN --action UNCOMMENT
    /etc/.custom/pfmanager.sh --tag @DHCP_OUT --action UNCOMMENT
}
...
if [ "$MY_IP_IS_PRIVATE" == "true" ]; then
    ...
    if [[ "$MY_IP" =~ ^169\.254\. ]]; then
        sleep 20
        /etc/.custom/ifmanager.sh --action RANDOM_MAC --include $INTERFACE
        /etc/.custom/ifmanager.sh --action UP --include $INTERFACE
